...
그러나 지금 현재, 진짜 도저히 왜 쓰는지 이해를 계속 못하는중.

이거 없다고 더 길어지는 코드가 있나?????
...
Go 언어에서 `type`은 새로운 데이터 타입을 정의하는 데 사용됩니다. `type`은 기존의 데이터 타입을 가지고 새로운 데이터 타입을 만들어낼 수 있습니다. 예를 들어, `type`을 사용하여 새로운 타입을 정의할 수 있습니다.
